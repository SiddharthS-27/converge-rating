...
const mapToFrontendOpportunity = (item: any): Opportunity => {
    ...
    return {
        ...
        teammates: teammates,
        // Map backend 'status' (e.g. "COMPLETED", "ACTIVE") to frontend
...
export const getTeammateRequests = async (): Promise<TeammateRequest[]> => {
    ...
    try {
        ...
        const data = await response.json();
        // Backend returns mixed requests now (Join vs Rating)
        // Ensure mapping handles extra fields if they exist
        return data.map((req: any) => ({
            ...req,
            type: req.type || 'JOIN_REQUEST', // Default for backward compatibility
            rateeId: req.rateeId,
            rateeName: req.rateeName
        }));
    } catch (error) {
...
export const completeProject = async (projectId: string | number): Promise<boolean> => {
    ...
    try {
        // This endpoint should now trigger the backend logic to create 'Rating Requests' for all members
 ...
 * ==============================================================================
 * Uploads extracted text from resume to update the user's profile.
 * Expects a PUT to /api/resume/update with JSON { resumeText: string }
 */
export const uploadUserResume = async (resumeText: string): Promise<boolean> => {
    const token = localStorage.getItem('token');
    if (!token) throw new Error("Authentication required.");
    ...
    try {
        const response = await fetch(`${API_URL}/resume/update`, {
            ...
            body: JSON.stringify({ 
                // ONLY sending extracted text, NO BASE64 PDF
        ...
        if (!response.ok) {
            const err = await response.text();
            throw new Error(err || "Failed to update resume.");
        }
        
        console.log("[Upload Service] Resume updated successfully.");
        return true;
    } catch (error) {
