...
export interface Opportunity {
  ...
  status?: 'ACTIVE' | 'COMPLETED'; // NEW: Track project completion
}

// Represents an incoming teammate request OR a rating request
export interface TeammateRequest {
    requestId: number;
    projectTitle: string;
    projectId?: number | string; // Helper for linking
    requesterEmail: string;
    status: 'PENDING' | 'ACCEPTED' | 'REJECTED';
    createdAt: string;
    
    // NEW FIELDS for Rating Workflow
    type?: 'JOIN_REQUEST' | 'RATING_REQUEST'; 
    rateeId?: number | string;   // The ID of the person being rated
    rateeName?: string;          // The name of the person being rated
}
